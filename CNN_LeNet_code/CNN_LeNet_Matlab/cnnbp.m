...
for L = (n - 1) : -1 : tmp
    ...
    if (strcmp(net.layers{L+1}.type, 'f'))
        ...
        %------------------------------------------------------------------
        % 以下代码对第6层(过渡全连接层)有效     #这里是不是写错了？ 第5层吧
            ...
            % 这里有问题啊，如果L层是s呢，那也要乘上其乘性偏置beta吧？注：这里不用乘，所以要清楚S层的Delta是缺了乘性偏置beta的，在用到的时候乘上即可。
    ...
    if (strcmp(net.layers{L+1}.type, 's'))        
        ...
        for J = 1 : net.layers{L}.iChannel                          % 对本层输出通道数做循环
            ...
            % 为上采样函数,这里用expand函数代替文献中的kron(kron仅适用二维情况)，下一层的上采样，复制到原来的尺寸
%             tmp2 = expand(net.layers{L + 1}.Delta{J}, [net.layers{L + 1}.iSample,net.layers{L + 1}.iSample,1]);   
    ...
    if (strcmp(net.layers{L+1}.type, 'c'))        
        ...
        for I = 1 : net.layers{L}.iChannel                          % 对本层输出通道数做循环
            ...
            % 注意，这里可能是输入层i，也可能是降采样层s，若为下采样层s，则其Delta本应乘上乘性偏置Beta，但是并没有乘，所以要知道S层的Delta是缺少乘性偏置Beta的，用到S层的Delta时补上即可。
            %下采样，没有sigmod的非线性关系，所以不需要
