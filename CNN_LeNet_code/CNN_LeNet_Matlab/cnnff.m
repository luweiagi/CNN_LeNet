...
net.layers{1}.X{1} = x;                                         % 网络的第一层就是输入(opts.batchsize幅输入同时处理),x32*32*10,10个一起算的
...
%             net.layers{L}.X{J} = fx(net.layers{L}.X{J});                % 本层输出计算
            ...
            % 特别注意:
            % 下采样层仅涉及两个个运算:(1)下采样, (2)偏置(乘和加),"sigmoid映射"这里都没有
        end
    ...
    if (strcmp(net.layers{L}.type, 'f'))
        ...
        % 以下代码对第6层(过渡全连接层)有效
            ...
            for J = 1 : net.layers{L-1}.iChannel                            % 对前一层输出通道数做循环
                ...
                % size(net.layers{L-1}.X{J}) = [5 5 10]
                % size(net.layers{L-1}.X_Array) = [25+25+25+25 10]
            end
            ...
            %size(net.layers{L}.X) = [120 10]
            %size(net.layers{L}.W) = [120 100]
            %size(net.layers{L-1}.X_Array) = [100 10]
            %size(net.layers{L}.B) = [120 1]
            %size(repmat(net.layers{L}.B, 1, size(net.layers{L-1}.X_Array, 2))) = [120 10]
            % 特别注意:
