...
void test()
{
	...
	//xxx.push_back(x1);
	xxx.at(1).at(1) = 9.9;
	cout << xxx.at(0).at(0) << endl;
	cout << xxx.at(0).at(1) << endl;

	xxx.print();
	return;

	//array2D a = create_array2D(3, 2, pow(0.5, 2));
	array2D a = rand_array2D(5, 4, 0, 100);
	...
	vector<array2D> vec_a;
	vec_a.assign(1, a);
	//

	array2D b = create_array2D(5, 4, 10);

	print_array2D(b);
	cout << endl;

	vector<array2D> vec_b;
	vec_b.assign(1, b);

	vector<array2D> vec_ab;

	vec_ab = get_A_dot_product_B_vector_array2D(vec_a, vec_b);

	print_array2D(vec_ab.at(0));
	cout << endl;

	return;

	vector<array2D> vec_a_pooling;

	vec_a_pooling = down_sample(vec_a, 2, MeanPooling);

	array2D a_pooling = vec_a_pooling.at(0);

	print_array2D(a_pooling);

	/*
	array2D X = create_array2D(8, 5, 1);
	...
	array2D Ker = create_array2D(3, 2, 1);

	vector<array2D> Y = activation_function(XX, SoftMax);// convolution_n_dim(XX, Ker);

	cout << endl;

	print_array2D(Y.at(0));

	cout << endl;

	print_array2D(Y.at(1));

	cout << endl;



	//array2D ddd = get_A_dot_product_B_array2D(Ker, Ker2);

	//print_array2D(ddd);

	//ddd[2][1] = 0;

	//cout << sum_of_array2D(ddd) << endl;

	/////////////////

	//cout << "conv" << endl;

	//array2D eee = convolution_one_dim(X, Ker);

	//print_array2D(eee);

	vector<array2D> bbb = convolution_n_dim(aaa, Ker);

	print_array2D(aaa.at(0));

	cout << endl;

	print_array2D(bbb.at(0));
	*/
